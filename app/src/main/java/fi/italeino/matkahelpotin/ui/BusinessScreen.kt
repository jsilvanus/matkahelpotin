...
fun BusinessScreen(viewModel: BusinessViewModel) {
    ...
    LaunchedEffect(employments) {
        if (employmentId == null) employmentId = employments.firstOrNull()?.id
    }
    ...
    val visibleRoutes = routes.filter { route ->
        ...
        allowed.isEmpty() || (route.fromPlaceId in allowed && route.toPlaceId in allowed)
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = { TextButton(onClick = { setup = !setup }) { Text(if (setup) "Week" else "Setup") } },
            )
        },
    ) { padding ->
        Column(
            ...
            if (setup) {
                BusinessSetup(viewModel, selectedEmployment!!, places, locations, routes, placeNames)
            } else {
...
@Composable
private fun EmploymentSelector(
                    ...
                    Card {
                        Column(Modifier.padding(8.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            ...
                            Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                                TextButton(onClick = { viewModel.removeOne(date, route, returnOnly = false) }) { Text("−") }
                                Text("$oneWayCount")
                                TextButton(onClick = { viewModel.addOneWay(date, employmentId, route) }) { Text("+") }
                            }
                            Row(verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) {
                                TextButton(onClick = { viewModel.removeOne(date, route, returnOnly = true) }) { Text("−") }
...
private fun BusinessSetup(
    ...
    var locationPlace by remember { mutableStateOf<EntityId?>(null) }
    ...
    places.forEach { place ->
        ...
        if (!already) {
            OutlinedButton(onClick = { viewModel.addBusinessLocation(employment.id, place.id, place.name) }) {
                Text("Add ${place.name}")
            }
        }
    }

    val businessPlaces = locations.filter { it.employmentId == employment.id && it.active }.mapNotNull { location -> places.firstOrNull { it.id == location.placeId } }
    ...
    OutlinedTextField(value = distance, onValueChange = { distance = it }, label = { Text("One-way distance (km)") })
    Button(
        enabled = routeFrom != null && routeTo != null && distance.replace(',', '.').toDoubleOrNull()?.let { it > 0 } == true,
        onClick = { viewModel.addRoute(routeFrom!!, routeTo!!, distance); distance = "" },
    ) { Text("Add employer-defined route") }

    routes.forEach { route ->
        Text("${placeNames[route.fromPlaceId]} → ${placeNames[route.toPlaceId]}: ${route.distanceMeters / 1000.0} km")
    }
}
