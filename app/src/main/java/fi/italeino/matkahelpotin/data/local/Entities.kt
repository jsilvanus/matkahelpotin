...
data class MileagePolicyEntity(@PrimaryKey val id: java.util.UUID, val year: Int, val mileageLimitMeters: Long?, val mileageRateCentsPerKm: Long, val jurisdiction: String, val scope: String, val validFrom: LocalDate, val validUntil: LocalDate?)
...
@Dao interface CommuteRecordDao { @Query("SELECT * FROM commute_record ORDER BY date") fun observeAll(): Flow<List<CommuteRecordEntity>>; @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun upsert(value: CommuteRecordEntity); @Query("DELETE FROM commute_record WHERE id = :id") suspend fun delete(id: java.util.UUID); @Query("DELETE FROM commute_record WHERE date = :date") suspend fun deleteByDate(date: LocalDate): Int }
